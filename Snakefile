...
#                     recursive=True)
SAMPLES = glob.glob(f"{RAW_DATA}**/*.fq.gz",
                    recursive=True)
SAMPLES = [sample.replace(f"{RAW_DATA}/","").replace(".fq.gz","") 
           for sample
           in SAMPLES]
SAMPLES = [sample.split(".")[0]
           for sample
           in SAMPLES]
SAMPLES = [_.split("/")[-1]
           for _
...
rule salmon_quant:
    """Pseudoalign sequences using Salmon. MUCH faster than STAR and 
